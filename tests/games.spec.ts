...
test('Add game button is visible when a user does not have this game saved', async ({ page }) => {
  ...
  await expect.soft(page.getByTestId('Add game')).toBeVisible();
  await expect.soft(page.getByTestId('Remove game')).not.toBeVisible();
  await expect.soft(page.getByTestId('chat_link_button')).not.toBeVisible();
});
...
test('Remove and Connect game buttons are visible when a user has this game saved', async ({ page }) => {
  ...
  await expect.soft(page.getByTestId('Remove game')).toBeVisible();
  await expect.soft(page.getByTestId('chat_link_button')).toBeVisible();
  await expect.soft(page.getByTestId('Add game')).not.toBeVisible();
});
...
test('Chat button takes user to game chat forum', async ({ page }) => {
  ...
  await page.getByTestId('chat_link_button').click();
