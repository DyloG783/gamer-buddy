...
test('Test Connected WITH test user is present', async ({ page }) => {
  ...
  await expect(connection.getByTestId('connection_card')).toContainText(automation_users[3].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[2].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[1].userName);
});
...
test('Test Connection REQUESTS with test user is present', async ({ page }) => {
  ...
  await expect(connection.getByTestId('connection_card')).toContainText(automation_users[2].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[3].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[1].userName);
});
...
test('Test FOLLOWING with test user is present', async ({ page }) => {
  ...
  await expect(connection.getByTestId('connection_card')).toContainText(automation_users[1].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[3].userName);
  await expect.soft(connection.getByTestId('connection_card')).not.toContainText(automation_users[2].userName);
});
